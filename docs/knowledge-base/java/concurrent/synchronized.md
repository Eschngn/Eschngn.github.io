...
引入轻量级锁的主要目的是：在多线程竞争不激烈的情况下，利用 CAS 来完成加锁操作，从而避免使用操作系统层面的重量级锁，减少线程切换带来的性能开销。
...
假设线程 `T1` 尝试获取某个对象 `obj` 的锁：

1. JVM 在 `T1` 的栈帧中为 `obj` 分配一个锁记录空间 `Lock Record`，用于保存锁的状态信息。

2. 将 `obj` 的 Mark Word 复制到线程栈中的锁记录中，用于后续解锁时恢复。

3. JVM 使用 CAS 操作尝试将 `obj` 的 Mark Word 更新为指向当前线程栈中锁记录的指针：

    - 如果 CAS 成功：表示当前线程成功获得轻量级锁，`obj` 对象头的 Mark Word 现在指向 `T1` 的 `Lock Record`；不涉及线程阻塞，性能很高

    - 如果 CAS 失败：表示此时已有其他线程持有锁，JVM 会判断是否出现真正的竞争，若竞争激烈，有两条以上的线程竞争同一个锁，则锁膨胀升级为重量级锁，线程进入阻塞

#### 解锁

线程 `T1` 执行完同步代码块后，开始释放轻量级锁：

1. JVM 从 `T1` 栈中的 `Lock Record` 取出保存的原始 Mark Word
...
3. JVM 从 T1 线程栈中删除该 `Lock Record`
